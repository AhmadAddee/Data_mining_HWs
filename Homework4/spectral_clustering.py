...
def load_graph(path):
    ...
    # Build unweighted symmetric adjacency (similarity) matrix A
    row_idx = []
    col_idx = []
    data = []
    for (u, v) in edges:
...
def spectral_clustering(A, k, return_eigenvalues=False):
    """
...
def estimate_k_from_eigengap(A, max_k=10):
    """
    ...
    k_hat = np.argmax(gaps[:max_k]) + 1     # +1 because gap[i] is between lambda_i and lambda_i+1
